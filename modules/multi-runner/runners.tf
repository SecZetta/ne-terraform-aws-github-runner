module "runners" {
  ...
  vpc_id        = coalesce(each.value.runner_config.vpc_id, module.vpc_main.vpc_id)
  subnet_ids    = coalesce(each.value.runner_config.subnet_ids, module.vpc_main.public_subnets)
